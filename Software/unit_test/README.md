...
Отдельные минимальные PlatformIO-проекты для проверки железа на стенде. Каждый каталог — **самостоятельный** проект.

Из **корня репозитория** `rov_academic` перейдите в `Software/unit_test/<имя_проекта>`, затем `pio run`. Подробности по пинам, скорости порта и загрузке — в **README.md** соответствующего каталога.
