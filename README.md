...
flowchart LR
  ...
  CT -.->|"RC-PWM<br/>опционально"| GR["Манипулятор<br/>gripper"]
...
| Борт → манипулятор | RC-PWM (опц.) | вход `PA7` на плате gripper | Альтернатива серво на борту; DC-мотор на отдельной плате |
...
Распиновка каждого модуля — в `src/config.h` или `src/Config.h` соответствующего проекта (имя файла в репозитории с учётом регистра).
